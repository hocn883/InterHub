...
const MyApplyJob = () => {
  ...
  const loadApplications = async (pageNumber = 0) => {
    try {
      setLoading(true);
      setError("");
      ...
      setPage(pageData?.page ?? pageNumber);
  ...
  const statusCounts = useMemo(() => {
    return applications.reduce(
      (counts, application) => {
        ...
        if (application.status === STATUS.APPROVE) {
          counts.APPROVE += 1;
        }
            ...
            <div className="application-list">
              {filteredApplications.length > 0 ? (
                ...
                <div className="application-empty">
                  <h3>Không có ứng viên</h3>

                  <p>
                    Không có hồ sơ ứng viên trong mục này.
                  </p>
                </div>
              )}
            </div>
