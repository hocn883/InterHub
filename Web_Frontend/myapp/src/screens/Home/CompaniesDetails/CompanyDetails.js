...
function CompanyDetails() {
  ...
  const loadJobPage = async (pageNumber = 0) => {
    try {
      ...
      const response = await api.get(endpoints.employerJobs(employerId), {
        params: { page: pageNumber, size: 5 },
      });

      const result = response.data?.result;
      const openJobs = (result?.content || []).filter((job) => job.status === "OPEN");
  ...
  const loadReviewPage = async (pageNumber = 0) => {
    try {
      ...
      const response = await api.get(endpoints.employerReview(employerId), {
        params: { page: pageNumber, size: 5 },
      });
      ...
      setReviews((prev) => {
        const ids = new Set(prev.map((review) => review.id));
        return [...prev, ...data.filter((review) => !ids.has(review.id))];
      });

      setReviewTotalElements(result?.totalElements || 0);
  ...
  useEffect(() => {
    const loadCompany = async () => {
      try {
        ...
        const response = await api.get(endpoints.employerDetails(employerId));
        setCompany(response.data?.result || null);
      } catch (error) {
        console.error("Load company error:", error);
        setError(error.response?.data?.message || "Không thể tải thông tin doanh nghiệp.");
      } finally {
        setLoading(false);
      }
    };

    if (employerId) loadCompany();
  }, [employerId]);
  ...
  useEffect(() => {
    const checkFollow = async () => {
      try {
        ...
        const response = await authApi(token).get(endpoints.followEmployer(employerId));
    ...
    if (employerId && currentUser?.role === "STUDENT") checkFollow();
  ...
  const handleFollow = async () => {
    try {
      ...
      const token = localStorage.getItem("access-token");
      await authApi(token).post(endpoints.followEmployer(employerId));

      setFollow(true);
    } catch (error) {
      console.error("Follow error:", error);
      alert(error.response?.data?.message || "Không thể theo dõi doanh nghiệp.");
    } finally {
  ...
  const handleUnFollow = async () => {
    try {
      ...
      const token = localStorage.getItem("access-token");
      await authApi(token).delete(endpoints.followEmployer(employerId));

      setFollow(false);
    } catch (error) {
      console.error("Unfollow error:", error);
      alert(error.response?.data?.message || "Không thể hủy theo dõi doanh nghiệp.");
    } finally {
      setFollowLoading(false);
    }
  };
  ...
  const handleChangeTab = async (tab) => {
    ...
    if (tab === "REVIEWS" && !reviewInitialized) {
      ...
      if (!success) setReviewInitialized(false);
    }
  };
  ...
  const handleLoadMoreReviews = async () => {
    ...
    if (success) setReviewPage(nextPage);
  };
  ...
  return (
    <div className="company-detail-page">
      <div className="company-detail-container">
        <button type="button" className="company-back-button" onClick={() => navigate(-1)}>
          ← Quay lại
        </button>
        ...
        <section className="company-profile-card">
          <div className="company-profile-header">
            <div className="company-logo">
              {company.avatarUrl ? (
                <img src={company.avatarUrl} alt={company.companyName} />
              ) : (
                <span>{company.companyName?.charAt(0).toUpperCase() || "C"}</span>
              )}
            </div>

            <div className="company-main-info">
              <div className="company-title">
                <h1>{company.companyName || "Doanh nghiệp"}</h1>
                <span
                  className={
                ...
                >
                  {company.status === "APPROVED" ? "Đã xác thực" : "Chờ xác thực"}
                </span>
              </div>
          ...
          <div className="company-information">
            <div className="company-info-item">
              ...
              <strong>{company.fullName || "Chưa cập nhật"}</strong>
            </div>

            <div className="company-info-item">
              <span>Tên đăng nhập</span>
              <strong>{company.username || "Chưa cập nhật"}</strong>
            </div>

            <div className="company-info-item">
              <span>Mã số thuế</span>
              <strong>{company.taxCode || "Chưa cập nhật"}</strong>
            </div>
        ...
        <section className="company-section">
          ...
          <p className="company-description">
            <strong>{company.companyName}</strong> đang tham gia nền tảng InterHub với vai trò
            nhà tuyển dụng, cung cấp các cơ hội thực tập và việc làm dành cho sinh viên.
          </p>
        </section>
          ...
          <section className="company-section company-tab-content">
            <div className="company-job-heading">
              <div className="company-section-header">
                ...
                <p>
                  Các vị trí hiện đang được <strong>{company.companyName}</strong> tuyển dụng.
                </p>
              </div>

              {jobs.length > 0 && (
                <div className="company-job-count">{jobs.length} vị trí</div>
              )}
            </div>

            {jobLoading && jobs.length === 0 ? (
              <div className="company-job-loading">Đang tải danh sách việc làm...</div>
            ) : jobs.length === 0 && !jobHasMore ? (
              <div className="company-job-empty">
                <h3>Chưa có vị trí tuyển dụng</h3>
                <p>Doanh nghiệp hiện chưa có công việc nào đang tuyển.</p>
              </div>
            ) : (
              <>
                <div className="company-job-list">
                  {jobs.map((job) => (
                    <JobCard
                      key={job.id}
                      job={{ ...job, employer: job.employer || company }}
                    />
                  ))}
                </div>
                ...
                {jobHasMore && !jobLoading && (
                  <div className="company-load-more">
                    <button type="button" onClick={handleLoadMoreJobs}>
                      Xem thêm công việc
                    </button>
                  </div>
                )}
          ...
          <section className="company-section company-tab-content">
            <div className="company-review-heading">
              <div className="company-section-header">
                ...
                <p>Trải nghiệm thực tập và làm việc tại doanh nghiệp.</p>
              </div>

              {reviewTotalElements > 0 && (
                <div className="review-count">{reviewTotalElements} đánh giá</div>
              )}
            </div>

            {reviewLoading && reviews.length === 0 ? (
              <div className="company-job-loading">Đang tải đánh giá...</div>
            ) : reviews.length === 0 ? (
              <div className="company-job-empty">
                <h3>Chưa có đánh giá</h3>
                <p>Doanh nghiệp hiện chưa có đánh giá từ sinh viên.</p>
              </div>
            ) : (
              <>
                    ...
                    <div className="review-card" key={review.id}>
                      <div className="review-top">
                        <div className="review-user">
                          ...
                          <div>
                            <h3>{review.studentName || "Sinh viên"}</h3>
                            <span>
                              {review.createdDate
                ...
                {reviewHasMore && !reviewLoading && (
                  <div className="company-load-more">
                    <button type="button" onClick={handleLoadMoreReviews}>
                      Xem thêm đánh giá
                    </button>
                  </div>
                )}
